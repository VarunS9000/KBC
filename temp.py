...
from tkinter import *
master= Tk()
w = Canvas(master,width=500,height=300,bg='dark blue')
w.pack()
canvas_height=10
canvas_width=200
...
if(a==5):
    top=Toplevel()
    top.title("KBC")
    t=Text(top,height=20,width=30,bg='light grey')
    t.pack()
    t.insert(END,"LOCK KIYA JAE?")
    tot=tkinter.Button(top,text="HAAN JI",width=30,command=top.destroy,activeforeground='red')
    tot.place(x=8,y=30)
mainloop()
...
def wow():
    ...
    t.pack()
    t.insert(END,"LOCK KIYA JAE?")
    tot=tkinter.Button(top,text="HAAN JI",width=27,command=top.destroy,activeforeground='black',bg='light cyan',font=('Airal',12))
    tot.place(x=8,y=30)
...
photo = PIL.ImageTk.PhotoImage(image = PIL.Image.fromarray(cv_img))
canvas.create_image(0, 0, image=photo, anchor=tkinter.NW)
ourMessage = "IS MY DADDY THE BEST??"
